...
class DatabaseProvider with ChangeNotifier {
  ...
  Future<void> register(Users user) async {
    final db = await database;
    await db.transaction((txn) async {
      await txn.insert(uTable,
          {'username': user.username, 'password': user.password}).then((_) {});
    });
  }
  ...
  Future<void> deleteCategory(String name) async {
    ...
    return await db.transaction((txn) async {
      return await txn
          .delete(cTable, where: 'title == ?', whereArgs: [name]).then((data) {
        _categories =
            _categories.where((element) => element.title != name).toList();
        notifyListeners();
      });
    });
  }
